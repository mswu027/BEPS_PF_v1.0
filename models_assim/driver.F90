...
integer              :: yr,mn,dy,tod,caldy,n_meteo,n_lai
...
real(r8)             :: secs_elapsed,secs_meteo,days_lai,secs_gpp_pf
...
PFweightupdatesum_resample=0.
 ...
 !**********************************************************************************run BEPS with dufalut parameters**************************************************
if (run_pf==0) then
  ...
  do     !! start time looping
    ...
    if (meteo_input >= 0) then  ! call hourly meteo. input
      if (nscale == 0) then
          ...
          call timemgr_diff_secs(2013*10000+1*100+1,0,yr*10000+mn*100+dy,tod,&
            secs_meteo)
 ...
 !call read_prior_para()       ! put the parameters to be optimized in a NETCDF
                                !file and read them as well their
                                !uncertainties,@MOUSONG.WU,2019-11
 call Create_PF_para(1,2,parloop) !"1" is PFT,"2" is soil texture !!create particles. xiuli
 nd = 0
 do     !! start time looping
    nd = nd + 1
    !-- iLab::inserted in order to pass 'calday' downstream
    ...
    call timemgr_diff_secs(2013*10000+1*100+1,0,yr*10000+mn*100+dy,tod,&
            secs_gpp_pf)
    n_meteo = int(secs_gpp_pf/3600 + 1)
    call read_PF_obs(n_meteo) !xiuli
    ...
    if (PF_obsva%obs_GPP(1) == PF_obsva%obs_GPP(1)) then !if observation exsits xiuli
     ...
     !do p =1,10!!1,nparameters !! start parameter loop
       ...
       do i = 1,npoints    !! spatial iteration
        ...
        call PF_weight(p1,yr, mn, dy, tod,get_nstep(), is_end_curr_month())
