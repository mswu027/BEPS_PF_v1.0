...
CONTAINS
    ...
    REAL(r8) function fMAP(nArray, Dataobs, Datasim, dFillValue)
        ...
        m = 0
        do i = 1, nArray
            if (Dataobs(i) .ne. dFillValue) then
                m = m + 1
                iData(m) = i
                !            sum11 = sum11 + Dataobs(i)
            end if
    ...
    REAL(r8) function fLxy(nArray, xx, yy, dFill)
        ...
        if (k < 1) then
            ...
            fLxy = sumxy - sumx * sumy/k !!FuncLxy = sumxy - sumx*sumy/nArray
        end if
        return
    END
    !------------------------------------------------------------------
    ...
    SUBROUTINE CountUniqueElement(n_in,array_in,n_out,array_out)
        ...
        INTEGER, intent(in) :: array_in(n_in)
        INTEGER, intent(out):: n_out
        INTEGER, intent(out):: array_out(n_in,2)  !!Unique Element, # of Unique Element
        
        !!LOCAL VARIABLES:
        INTEGER k
        
        call sort1(n_in,array_in)
        n_out = 1
        array_out(:,:) = 0
        array_out(1,1) = array_in(1)
        array_out(1,2) = 1       
        do k = 2, n_in
            if(array_in(k).ne.array_out(n_out,1)) then
                n_out = n_out + 1
                array_out(n_out,1) = array_in(k)                
            end if
            array_out(n_out,2) = array_out(n_out,2) + 1
        end do
