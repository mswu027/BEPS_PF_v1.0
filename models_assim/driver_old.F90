 ...
 do     !! start time looping
    ...
    if (PF_obsva%obs_VOD(1) == PF_obsva%obs_VOD(1)) then
       ...
       do i = 1,npoints    !! spatial iteration
          ...
          do j = 11,11!1,PFT    !! PFT iteration
               ...
               if(bfields%lcno(i,j) > 0 .and. bfields%sw(i) >0. .and. bfields%stext(i) >0 .and. bfields%clumping(i) > 0.5) then
                    ...
                    write(*,*) 'land cover = ',bfields%lcno(i,j)
                    write(*,*) 'soil texture = ',bfields%stext
                    write(*,*) 'laiyr = ',bfields%laiyr(i,j)
                    write(*,*) 'f_decay = ', ppar%p_f_decay(j,i)    ! initial value should be 1, for a scaling factor, but ouptput of 0.

                    if(nsrest == nsrStartup .and. is_first_step()) then
                        write(*,*) 'Startup!!!!'
                        call Init_soil_parameters(bfields%lcno(i,j),bfields%laiyr(i,j),bfields%stext(i),&  !  ,,
                          ppar%p_f_decay(j,i), param(27),soilp)
                        ...
                        write(*,*) 'Continue!!!!'
                        call Init_soil_parameters(bfields%lcno(i,j),bfields%laiyr(i,j),bfields%stext(i),&
                           ppar%p_f_decay(j,i), param(27),soilp) !need to check 2023/07/07
                        soilp%r_drainage = param(26)
                        !soilp%r_drainage = ppar%p_drainage(j)  ! read from nc
                                                          ! file,MOUSONG.WU@2019-11
                        !ii = bfields%stext(i)
                        ...
                        call retrive_soilp(soilp,i,j,0)
                        write(*,*) 'layer = ', soilp%n_layer
                        ! write(*,*) 'thetam_prev = ', soilp%thetam_prev(1)
                    end if
                    !print *, 'end of retrive_soilp'
                    !!! simulating photosynthesis
                    !              do llll = 0,40
                    ...
                    call inter_prg(yr, mn, dy, tod, &
                         ...
                         ppar%p_VN_slope(p1,i),PF_ppar%b_h2o(p1,i),PF_ppar%m_h2o(p1,i),PF_ppar%f_leaf(p1,i),&
     ...
     if (SS<parloop*0.95) then
        ...
        inparticles(:,18)=PF_ppar%f_resp(:,1)
        inparticles(:,19)=PF_ppar%f_decay(:,1)
        inparticles(:,20)=PF_ppar%a(:,1)
        inparticles(:,21)=PF_ppar%b(:,1)
        inparticles(:,22)=PF_ppar%c(:,1)

        inparticles(:,23)=PF_ppar%pfweight(:,1)
        ...
        PF_ppar%f_resp(:,1)=PF_resa%outparticles(:,18)
        PF_ppar%f_decay(:,1)=PF_resa%outparticles(:,19)
        PF_ppar%a(:,1)=PF_resa%outparticles(:,20)
        PF_ppar%b(:,1)=PF_resa%outparticles(:,21)
        PF_ppar%c(:,1)=PF_resa%outparticles(:,22)
        PF_resa%resample_weight(:)=PF_resa%outparticles(:,23)
