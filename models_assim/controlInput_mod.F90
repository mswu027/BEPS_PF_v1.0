...
module controlInput_mod
  ...
  character(len=255) :: beps_PF_obs_path
...
contains
  ...
  subroutine read_prior_para()
   ...
   ! call check(nf90_get_var(ncid,varid(14),p_f_lr))
    ...
    assim%p_N_leaf = p_N_leaf
    assim%p_r_decay = p_r_decay
   ...
   ! assim%p_f_lr = p_f_lr
    ...
    assim%u_N_leaf = u_N_leaf
    assim%u_r_decay = u_r_decay
   ...
   ! call check(nf90_get_var(ncid,varid(14),p_f_lr))
    ...
    do m=1,nlp
     do n=1,parloop
      ...
      !write(*,*) 'u_Vcmax(j,m)=', u_Vcmax(j,m)
      !write(*,*) 'PF%Vcmax=', PF%Vcmax(n,m)
      PF%Vcmax(n,m) = p_Vcmax(j,m)+(u_Vcmax(j,m)-p_Vcmax(j,m))*array(n)
      !write(*,*) 'PF%Vcmax=', PF%Vcmax(n,m)
      PF%q10(n,m) = p_q10(j,m)+(u_q10(j,m)-p_q10(j,m))*array(n)
      PF%VJ_slope(n,m) = p_VJ_slope(j,m)+(u_VJ_slope(j,m)-p_VJ_slope(j,m))*array(n)
      !write(*,*) 'p_N_leaf(j,m)=', p_N_leaf(j,m)
      !write(*,*) 'u_N_leaf(j,m)=', u_N_leaf(j,m)
      PF%N_leaf(n,m) = p_N_leaf(j,m)+(u_N_leaf(j,m)-p_N_leaf(j,m))*array(n)
      PF%r_decay(n,m) =  p_r_decay(j,m)+(u_r_decay(j,m)-p_r_decay(j,m))*array(n)
      PF%b_h2o(n,m) = p_b_h2o(j,m)+(u_b_h2o(j,m)-p_b_h2o(j,m))*array(n)
      PF%sif_alpha(n,m) = p_sif_alpha(j,m)+(u_sif_alpha(j,m)-p_sif_alpha(j,m))*array(n)
      PF%sif_beta(n,m) =  p_sif_beta(j,m)+(u_sif_beta(j,m)-p_sif_beta(j,m))*array(n)
      PF%taweff(n,m) =  p_taweff(j,m)+(u_taweff(j,m)-p_taweff(j,m))*array(n)
      PF%D0(n,m) = p_D0(j,m)+(u_D0(j,m)-p_D0(j,m))*array(n)
      PF%Ksat_scalar(n,m) = p_Ksat_scalar(ii,m)+(u_Ksat_scalar(ii,m)-p_Ksat_scalar(ii,m))*array(n)
      PF%b_scalar(n,m) = p_b_scalar(ii,m)+(u_b_scalar(ii,m)-p_b_scalar(ii,m))*array(n)
      PF%m_h2o(n,m) =  p_m_h2o(j,m)+(u_m_h2o(j,m)-p_m_h2o(j,m))*array(n)
      PF%f_leaf(n,m) =  p_f_leaf(j,m)+(u_f_leaf(j,m)-p_f_leaf(j,m))*array(n)
      PF%kc25(n,m) =  p_kc25(j,m)+(u_kc25(j,m)-p_kc25(j,m))*array(n)
      PF%ko25(n,m) = p_ko25(j,m)+(u_ko25(j,m)-p_ko25(j,m))*array(n)
      PF%tau25(n,m) =  p_tau25(j,m)+(u_tau25(j,m)-p_tau25(j,m))*array(n)
      ! assim%p_f_lr = p_f_lr
      PF%agb2vod(n,m) =  p_agb2vod(j,m)+(u_agb2vod(j,m)-p_agb2vod(j,m))*array(n)
      PF%pfweight(n,m) =  1./parloop
     end do
    end do
