...
! Public methods
 ...
 !     timemgr_restart_io,       &! read/write time manager restart info and restart time manager
      ...
      !--iLab::added
      get_doys,                 &! number of days per year (taking into account calendar settinngs)
...
! Arguments
   integer, optional, intent(in) :: offset  ! Offset from current time in seconds.
                                            ! Positive for future times, negative
                                            ! for previous times.
   !--iLab::calling context(s) expect an integer value to be returned,
   !        thus return type was adjusted.
! Return value
   integer :: get_curr_calday
   real(r8) :: get_curr_calday_r8
   ! Local variables
...
!-----------------------------------------------------------------------------------------
   ...
   get_curr_calday = int(get_curr_calday_r8)
...
!-- iLab::added routine that computes delta (in seconds) between two time-points
...
!--iLab::added for encapsulation code
function get_doys(yr) result(doys)
  implicit none
