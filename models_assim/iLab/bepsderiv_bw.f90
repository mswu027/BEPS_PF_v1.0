...
SUBROUTINE COS_PLANT_BW(lc, cosa, cosa_bw, convf, g_sw, g_sw_bw, g_b, &
& g_b_bw, vcmax, vcmax_bw, ffpa, ffpa_bw, f_soilwater, f_soilwater_bw, &
& cos_assim, cos_assim_bw)
...
! mol/m2/s
  ...
  END IF
! m/s
  gtcos = 1.0/(1.94/(g_sw*convf)+1.0/(g_b*convf)+1.0/(gcosm*convf))
! pmol/m2/s
  ...
  temp_bw0 = -(gtcos_bw/temp2**2)
  g_sw_bw = g_sw_bw - temp1*temp_bw0/g_sw
  g_b_bw = g_b_bw - temp0*temp_bw0/g_b
  gcosm_bw = -(temp*temp_bw0/gcosm)
...
! ppb
    ...
    CALL COS_PLANT_BW(lc, cosii, cosii_bw, convf, gs_h2o_mole, &
&               gs_h2o_mole_bw, g_lb_cos, g_lb_cos_bw, vcmax, vcmax_bw, &
&               ffpa, ffpa_bw, f_soilwater, f_soilwater_bw, cos_assim, &
&               cos_assim_bw)
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      ci_bw = 0.0_8
...
&           temp_bw0
      ...
      DO i=p%n_layer-1,0,-1
        ...
        IF (.NOT.(temp4 .LE. 0.0_8 .AND. (-p%b(i) .EQ. 0.0_8 .OR. -p%b(i&
&           ) .NE. INT(-p%b(i))))) p_bw%thetab(i) = p_bw%thetab(i) - p%b&
&           (i)*temp4**((-1)-p%b(i))*p%psi_sat(i)*p_bw%psib(i)/p%fei(i)
        IF (.NOT.temp4 .LE. 0.0_8) p_bw%b(i) = p_bw%b(i) - temp4**(-p%b(&
&           i))*LOG(temp4)*p%psi_sat(i)*p_bw%psib(i)
        p_bw%psib(i) = 0.0_8
      END DO
      DO i=p%n_layer-1,0,-1
        CALL POPCONTROL1B(branch)
        ...
        IF (.NOT.(temp4 .LE. 0.0_8 .AND. (-p%b(i) .EQ. 0.0_8 .OR. -p%b(i&
&           ) .NE. INT(-p%b(i))))) p_bw%thetam(i) = p_bw%thetam(i) - p%b&
&           (i)*temp4**((-1)-p%b(i))*p%psi_sat(i)*p_bw%psim(i)/p%fei(i)
        IF (.NOT.temp4 .LE. 0.0_8) p_bw%b(i) = p_bw%b(i) - temp4**(-p%b(&
&           i))*LOG(temp4)*p%psi_sat(i)*p_bw%psim(i)
        p_bw%psim(i) = 0.0_8
      END DO
      DO i=p%n_layer-1,0,-1
        CALL POPCONTROL1B(branch)
...
&           )*ft_bw(i)
      ...
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        temp = (p%psim(i)-p%psi_min)/p%psi_min
        temp0 = temp**p%alpha + 1.
        IF (.NOT.(temp .LE. 0.0_8 .AND. (p%alpha .EQ. 0.0_8 .OR. p%alpha&
&           .NE. INT(p%alpha)))) p_bw%psim(i) = p_bw%psim(i) - p%alpha*&
&           temp**(p%alpha-1)*fpsisr_bw(i)/(p%psi_min*temp0**2)
        fpsisr_bw(i) = 0.0_8
      ELSE
        fpsisr_bw(i) = 0.0_8
      END IF
    ...
    DO i=p%n_layer-1,0,-1
      ...
      END IF
      temp = p%thetam(i)/p%fei(i)
      IF (.NOT.(temp .LE. 0.0_8 .AND. (-p%b(i) .EQ. 0.0_8 .OR. -p%b(i) &
&         .NE. INT(-p%b(i))))) p_bw%thetam(i) = p_bw%thetam(i) - p%b(i)*&
&         temp**((-1)-p%b(i))*p%psi_sat(i)*p_bw%psim(i)/p%fei(i)
      IF (.NOT.temp .LE. 0.0_8) p_bw%b(i) = p_bw%b(i) - temp**(-p%b(i))*&
&         LOG(temp)*p%psi_sat(i)*p_bw%psim(i)
      p_bw%psim(i) = 0.0_8
    END DO
...
! arguments
  ...
  REAL(kind=8) :: x_bw(n), obsdiff_bw(m)
! local
...
! arguments
  ...
  REAL(kind=8) :: x_bw(n), priordiff_bw(n)
! local
