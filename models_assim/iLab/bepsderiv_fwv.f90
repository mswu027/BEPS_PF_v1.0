...
!  Tapenade 3.16 (feature/llhTests) -  5 Jul 2021 15:17
!
...
SUBROUTINE COS_PLANT_FWV(lc, cosa, cosa_fw, convf, g_sw, g_sw_fw, g_b, &
& g_b_fw, vcmax, vcmax_fw, ffpa, ffpa_fw, f_soilwater, f_soilwater_fw, &
& cos_assim, cos_assim_fw, nbdirs)
...
&     ))+vcmax*ffpa*f_soilwater_fw(nd))
  ...
  END IF
! m/s
  temp = 1.0/(convf*gcosm)
  temp0 = 1.0/(convf*g_b)
  temp1 = 1.94/(convf*g_sw)
  temp2 = 1.0/(temp1+temp0+temp)
  gtcos = temp2
  DO nd=1,nbdirs
    gtcos_fw(nd) = -(temp2*(-(temp1*g_sw_fw(nd)/g_sw)-temp0*g_b_fw(nd)/&
&     g_b-temp*gcosm_fw(nd)/gcosm)/(temp1+temp0+temp))
! pmol/m2/s
...
&       (/nlp, 1/))
      CALL CHECK(result1)
      result1 = NF90_GET_VAR(ncid, varid(2), rh, start=(/1, nd/), count=&
&       (/nlp, 1/))
      CALL CHECK(result1)
      result1 = NF90_GET_VAR(ncid, varid(3), ws, start=(/1, nd/), count=&
&       (/nlp, 1/))
      CALL CHECK(result1)
...
! parameters used for COS calculations
    REAL(r8) :: cosa, convf, cosi, cosii, coss
...
! add the module for calculating COS uptake by plants, @MOUSONG.WU 2020-09-17
    CALL COS_PLANT_FWV(lc, cosii, cosii_fw, convf, gs_h2o_mole, &
&                gs_h2o_mole_fw, g_lb_cos, g_lb_cos_fw, vcmax, vcmax_fw&
&                , ffpa, ffpa_fw, f_soilwater, f_soilwater_fw, cos_assim&
&                , cos_assim_fw, nbdirs)
    DO nd=1,nbdirs
! ppb
...
!    xSIF = iphoton
    END IF
! add the module for calculating COS uptake by plants, @MOUSONG.WU 2020-09-17
    CALL COS_PLANT(lc, cosii, convf, gs_h2o_mole, g_lb_cos, vcmax, ffpa&
...
!-1
  ...
  SUBROUTINE INIT_SOIL_ROOTFRACTION(p)
    ...
    pwy1 = cum_depth(0)*100
    pwr1 = p%r_root_decay**pwy1
    p%f_root(0) = 1 - pwr1
    DO i=1,p%n_layer-2
      cum_depth(i) = cum_depth(i-1) + p%d_soil(i)
      pwy1 = cum_depth(i-1)*100
      pwr1 = p%r_root_decay**pwy1
      pwy2 = cum_depth(i)*100
      pwr2 = p%r_root_decay**pwy2
      p%f_root(i) = pwr1 - pwr2
    END DO
    pwy1 = cum_depth(p%n_layer-2)*100
    p%f_root(p%n_layer-1) = p%r_root_decay**pwy1
    RETURN
...
! the thermal conductivity of water
    ...
    INTRINSIC MAX
    REAL(r8) :: pwy1
    REAL(r8), DIMENSION(nbdirsmax) :: pwy1_fw
    INTEGER :: nd
    REAL(r8) :: temp
    INTEGER :: nbdirs
    DO i=0,p%n_layer-1
! dry
      pwy1 = 1 - p%fei(i)
      tmp1 = p%thermal_cond(i)**pwy1
      pwy1 = 1.2*p%thetam(i)*p%ice_ratio(i)
      temp = ki**pwy1
      DO nd=1,nbdirs
!ice
        pwy1_fw(nd) = 1.2*(p%ice_ratio(i)*p_fw%thetam(nd, i)+p%thetam(i)&
&         *p_fw%ice_ratio(nd, i))
        IF (ki .LE. 0.0_8) THEN
          tmp2_fw(nd) = 0.0_8
        ELSE
          tmp2_fw(nd) = temp*LOG(ki)*pwy1_fw(nd)
        END IF
!water
        pwy1_fw(nd) = (1-p%ice_ratio(i))*p_fw%thetam(nd, i) - p%thetam(i&
&         )*p_fw%ice_ratio(nd, i)
!!Sr ??
        tmp4_fw(nd) = p_fw%thetam(nd, i)/p%fei(i)
      END DO
      tmp2 = temp
      pwy1 = p%thetam(i)*(1-p%ice_ratio(i))
      temp = kw**pwy1
      DO nd=1,nbdirs
        IF (kw .LE. 0.0_8) THEN
          tmp3_fw(nd) = 0.0_8
        ELSE
          tmp3_fw(nd) = temp*LOG(kw)*pwy1_fw(nd)
        END IF
      END DO
      tmp3 = temp
...
! infiltration, and Maximum infiltration
    ...
    REAL(r8) :: abs0
    REAL(r8) :: pwx1
    REAL(r8), DIMENSION(nbdirsmax) :: pwx1_fw
    REAL(r8) :: pwy1
    REAL(r8), DIMENSION(nbdirsmax) :: pwy1_fw
    REAL(r8) :: pwr1
    REAL(r8), DIMENSION(nbdirsmax) :: pwr1_fw
    INTEGER :: nd
    REAL(r8) :: temp
    REAL(r8) :: temp0
    REAL(r8) :: temp1
...
&       r_rain_g(nd)-infil_fw(nd))
    ...
    DO WHILE (total_t .LT. kkstep)
      DO i=0,p%n_layer-1
        pwx1 = p%thetam(i)/p%fei(i)
        pwy1 = 2.*p%b(i) + 3.
        temp = pwx1**pwy1
        pwr1 = temp
        DO nd=1,nbdirs
          pwx1_fw(nd) = p_fw%thetam(nd, i)/p%fei(i)
          pwy1_fw(nd) = 2.*p_fw%b(nd, i)
          IF (pwx1 .LE. 0.0_8 .AND. (pwy1 .EQ. 0.0_8 .OR. pwy1 .NE. INT(&
&             pwy1))) THEN
            pwr1_fw(nd) = 0.0_8
          ELSE IF (pwx1 .LE. 0.0_8) THEN
            pwr1_fw(nd) = pwy1*pwx1**(pwy1-1)*pwx1_fw(nd)
          ELSE
            pwr1_fw(nd) = pwy1*pwx1**(pwy1-1)*pwx1_fw(nd) + temp*LOG(&
&             pwx1)*pwy1_fw(nd)
          END IF
          p_fw%km(nd, i) = p%ksat(i)*(pwr1*p_fw%f_ice(nd, i)+p%f_ice(i)*&
&           pwr1_fw(nd)) + p%f_ice(i)*pwr1*p_fw%ksat(nd, i)
        END DO
        p%km(i) = p%f_ice(i)*p%ksat(i)*pwr1
      END DO
      DO i=0,p%n_layer-1
        IF (i .LT. p%n_layer - 1) THEN
          temp = 1.0/p%d_soil(i) + 1.0/p%d_soil(i+1)
          DO nd=1,nbdirs
            p_fw%thetab(nd, i) = (p_fw%thetam(nd, i+1)/p%d_soil(i+1)+&
&             p_fw%thetam(nd, i)/p%d_soil(i))/temp
          END DO
...
&               pwx1)*pwy1_fw(nd)
          ...
          END DO
          pwr1 = temp
          temp = p%f_ice(i)*pwr1/(p%d_soil(i)+p%d_soil(i+1))
          temp0 = p%d_soil(i)*p%ksat(i) + p%d_soil(i+1)*p%ksat(i+1)
          DO nd=1,nbdirs
            p_fw%kb(nd, i) = temp*(p%d_soil(i)*p_fw%ksat(nd, i)+p%d_soil&
&             (i+1)*p_fw%ksat(nd, i+1)) + temp0*(pwr1*p_fw%f_ice(nd, i)+&
&             p%f_ice(i)*pwr1_fw(nd))/(p%d_soil(i)+p%d_soil(i+1))
          END DO
          p%kb(i) = temp0*temp
        ELSE
          pwx1 = p%thetab(i)/p%fei(i)
...
&               INT(pwy1))) THEN
            ...
            ELSE IF (pwx1 .LE. 0.0_8) THEN
              pwr1_fw(nd) = pwy1*pwx1**(pwy1-1)*pwx1_fw(nd)
            ELSE
              pwr1_fw(nd) = pwy1*pwx1**(pwy1-1)*pwx1_fw(nd) + temp0*LOG(&
&               pwx1)*pwy1_fw(nd)
            END IF
            p_fw%kb(nd, i) = 0.5*(p%ksat(i)*(pwr1*p_fw%f_ice(nd, i)+p%&
&             f_ice(i)*pwr1_fw(nd))+p%f_ice(i)*pwr1*p_fw%ksat(nd, i))
          END DO
          p%kb(i) = 0.5*p%f_ice(i)*p%ksat(i)*pwr1
        END IF
      END DO
...
&             pwx1)*pwy1_fw(nd)
          END IF
          p_fw%psim(nd, i) = p%psi_sat(i)*pwr1_fw(nd)
        END DO
        pwr1 = temp0
        p%psim(i) = p%psi_sat(i)*pwr1
...
&             pwx1)*pwy1_fw(nd)
          END IF
          p_fw%psib(nd, i) = p%psi_sat(i)*pwr1_fw(nd)
        END DO
        pwr1 = temp0
        p%psib(i) = p%psi_sat(i)*pwr1
...
! the unsaturated hydraulic conductivity of soil p%n_layer
      DO i=0,p%n_layer-1
        IF (i .LT. p%n_layer - 1) THEN
! see seller's
          temp0 = p%b(i) + p%b(i+1) + 6
          temp = (p%psim(i)+p%psim(i+1))*temp0
          temp1 = (p%b(i)+p%b(i+1))/temp
          temp2 = p%km(i)*p%psim(i) + p%km(i+1)*p%psim(i+1)
          DO nd=1,nbdirs
            p_fw%kk(nd, i) = temp1*(p%psim(i)*p_fw%km(nd, i)+p%km(i)*&
&             p_fw%psim(nd, i)+p%psim(i+1)*p_fw%km(nd, i+1)+p%km(i+1)*&
&             p_fw%psim(nd, i+1)) + temp2*(p_fw%b(nd, i)+p_fw%b(nd, i+1)&
&             -temp1*(temp0*(p_fw%psim(nd, i)+p_fw%psim(nd, i+1))+(p%&
&             psim(i)+p%psim(i+1))*(p_fw%b(nd, i)+p_fw%b(nd, i+1))))/&
&             temp
          END DO
          p%kk(i) = temp2*temp1
        ELSE
          temp2 = (p%psim(i)+p%psib(i))*(p%b(i)+3)
          temp1 = p%b(i)/temp2
          temp0 = p%km(i)*p%psim(i) + p%kb(i)*p%psib(i)
          DO nd=1,nbdirs
            p_fw%kk(nd, i) = temp1*(p%psim(i)*p_fw%km(nd, i)+p%km(i)*&
&             p_fw%psim(nd, i)+p%psib(i)*p_fw%kb(nd, i)+p%kb(i)*p_fw%&
&             psib(nd, i)) + temp0*(p_fw%b(nd, i)-temp1*((p%b(i)+3)*(&
...
! downwards positive , +1 accounts for gravitational drainage LHE
          temp2 = 2*(p%psim(i+1)-p%psim(i))/(p%d_soil(i)+p%d_soil(i+1)) &
&           + 1
          DO nd=1,nbdirs
            p_fw%r_waterflow(nd, i) = temp2*p_fw%kk(nd, i) + p%kk(i)*2*(&
&             p_fw%psim(nd, i+1)-p_fw%psim(nd, i))/(p%d_soil(i)+p%d_soil&
&             (i+1))
          END DO
          p%r_waterflow(i) = p%kk(i)*temp2
        ELSE
! from Ju
          DO nd=1,nbdirs
...
! end do while
    DO i=0,p%n_layer-1
      temp2 = p%ice_ratio(i)*p%thetam_prev(i)/p%thetam(i)
      DO nd=1,nbdirs
        p_fw%ice_ratio(nd, i) = (p%thetam_prev(i)*p_fw%ice_ratio(nd, i)+&
&         p%ice_ratio(i)*p_fw%thetam_prev(nd, i)-temp2*p_fw%thetam(nd, i&
&         ))/p%thetam(i)
      END DO
      p%ice_ratio(i) = temp2
      IF (1.0 .GT. p%ice_ratio(i)) THEN
...
&           d_soil(i+1))/(p%d_soil(i)+p%d_soil(i+1))*pwr1
        ELSE
! i= n_layer-1
          pwx1 = p%thetab(i)/p%fei(i)
          pwy1 = 2.*p%b(i) + 3.
...
&           pwy1))) THEN
        ...
        ELSE IF (pwx1 .LE. 0.0_8) THEN
          pwr1_fw(nd) = pwy1*pwx1**(pwy1-1)*pwx1_fw(nd)
        ELSE
          pwr1_fw(nd) = pwy1*pwx1**(pwy1-1)*pwx1_fw(nd) + temp*LOG(pwx1)&
&           *pwy1_fw(nd)
        END IF
        p_fw%psim(nd, i) = p%psi_sat(i)*pwr1_fw(nd)
      END DO
    ...
    DO i=0,p%n_layer-1
      IF (p%psim(i) .GT. p%psi_min) THEN
        pwx1 = (p%psim(i)-p%psi_min)/p%psi_min
        pwr1 = pwx1**p%alpha
        DO nd=1,nbdirs
          pwx1_fw(nd) = p_fw%psim(nd, i)/p%psi_min
          IF (pwx1 .LE. 0.0_8 .AND. (p%alpha .EQ. 0.0_8 .OR. p%alpha &
&             .NE. INT(p%alpha))) THEN
            pwr1_fw(nd) = 0.0_8
          ELSE
            pwr1_fw(nd) = p%alpha*pwx1**(p%alpha-1)*pwx1_fw(nd)
          END IF
          fpsisr_fw(nd, i) = -(pwr1_fw(nd)/(pwr1+1.)**2)
        END DO
        fpsisr(i) = 1./(1.+pwr1)
      ELSE
        DO nd=1,nbdirs
...
&             .NE. INT(t2))) THEN
            pwr1_fw(nd) = 0.0_8
          ELSE
            pwr1_fw(nd) = t2*p%temp_soil_p(i)**(t2-1)*p_fw%temp_soil_p(&
&             nd, i)
          END IF
          arg1_fw(nd) = t1*pwr1_fw(nd)
...
!eq. 14 in Ju 2006
    ...
    ELSE
      DO i=0,p%n_layer-1
        temp = dtt(i)/dtt_sum
        DO nd=1,nbdirs
          p_fw%dt(nd, i) = (dtt_fw(nd, i)-temp*dtt_sum_fw(nd))/dtt_sum
        END DO
        p%dt(i) = temp
...
&       .OR. lc .EQ. 41) THEN
      pwr1 = q10**exponent1
      respir_root_o = biomass_root_o*coef_root_respir*pwr1*ra
    ELSE
      pwr1 = biomass_root_o**(-0.841)
      ratio_froot = EXP(1.007)*pwr1
...
!kgC/m2/s
      pwr1 = q10**exponent1
      respir_leaf_o = lai/lai_max_o*biomass_leaf_o*coef_leaf_respir*pwr1&
&       *ra
    END IF
...
&       .OR. lc .EQ. 41) THEN
      pwr1 = q10**exponent1
      respir_root_u = biomass_root_u*coef_root_respir*pwr1*ra
    ELSE
      pwr1 = biomass_root_u**(-0.841)
      ratio_froot = EXP(1.007)*pwr1
      IF (0.9 .GT. ratio_froot) THEN
        ratio_froot = ratio_froot
      ELSE
...
&       , dimid_lat, dimid_time/), varid)
      CALL CHECK(result1)
      result1 = NF90_PUT_ATT(ncid, varid, 'units', 'mol/m2/s')
      CALL CHECK(result1)
      result1 = NF90_PUT_ATT(ncid, varid, 'name', &
...
!tely.
  ...
  arg1 = f_opt/f_g
  arg2 = s_opt/s_g
  pwx1 = LOG(arg2) + (s_g/s_opt-1.)
  pwr1 = pwx1**(-1)
  a = LOG(arg1)*pwr1
  pwx1 = soil_s/s_opt
  pwr1 = pwx1**a
  arg1 = -(a*(soil_s/s_opt-1.))
  cos_soil_biotic = f_opt*pwr1*EXP(arg1)
...
!emissivity of each part
  ...
  pwx1 = e_actual*10.0
  pwy1 = (temp_air+273.15)/1200.0
  pwr1 = pwx1**pwy1
  emissivity_air = 1. - EXP(-pwr1)
...
! arguments
  INTEGER(kind=4) :: n, m
  REAL(kind=8) :: x(n), f
  REAL(kind=8) :: x_fw(nbdirsmax, n), f_fw(nbdirsmax)
! local
...
! arguments
  ...
  REAL(kind=8) :: x_fw(nbdirsmax, n), obsdiff_fw(nbdirsmax, m)
! local
...
! for calculation of windspeed at 50 m height
  ...
  ELSE
    ...
    ELSE
      pwx1 = 1. - 16.*(z_50-d)*l
      DO nd=1,nbdirs
        pwx1_fw(nd) = -((z_50-d)*16.*l_fw(nd))
        psi_fw(nd) = -(0.5*pwx1**(-1.5)*pwx1_fw(nd))
      END DO
      psi = pwx1**(-0.5)
    END IF
    IF (10.0 .GT. psi) THEN
...
&         1.0/3.))) THEN
        pwr1_fw(nd) = 0.0_8
      ELSE
        pwr1_fw(nd) = le**(1.0/3.-1)*le_fw(nd)/3.
      END IF
      gamma1_fw(nd) = (uh*0.179+0.167)*pwr1_fw(nd)
...
&       nd, j-1)
  ...
  DO nd=1,nbdirs
    ...
    pwx1_fw(nd) = arg2_fw(nd)/arg2 - s_g*s_opt_fw(nd)/s_opt**2
    pwr1_fw(nd) = -(pwx1_fw(nd)/pwx1**2)
    pwx1_fw(nd) = (soil_s_fw(nd)-soil_s*s_opt_fw(nd)/s_opt)/s_opt
  END DO
  soil_ice = soil_ice/dsoil
  cos_soil_abiotic = 0.437*EXP(arg1)
  arg1 = f_opt/f_g
  ...
  temp = LOG(arg1)
  DO nd=1,nbdirs
    a_fw(nd) = pwr1*arg1_fw(nd)/arg1 + temp*pwr1_fw(nd)
  END DO
  a = temp*pwr1
  pwx1 = soil_s/s_opt
  temp = pwx1**a
  DO nd=1,nbdirs
    IF (pwx1 .LE. 0.0_8 .AND. (a .EQ. 0.0_8 .OR. a .NE. INT(a))) THEN
      pwr1_fw(nd) = 0.0_8
    ELSE IF (pwx1 .LE. 0.0_8) THEN
      pwr1_fw(nd) = a*pwx1**(a-1)*pwx1_fw(nd)
    ELSE
      pwr1_fw(nd) = a*pwx1**(a-1)*pwx1_fw(nd) + temp*LOG(pwx1)*a_fw(nd)
    END IF
  END DO
  pwr1 = temp
  temp = soil_s/s_opt
  DO nd=1,nbdirs
    arg1_fw(nd) = -((temp-1.)*a_fw(nd)+a*(soil_s_fw(nd)-temp*s_opt_fw(nd&
&     ))/s_opt)
  END DO
  arg1 = -(a*(temp-1.))
  temp = EXP(arg1)
  DO nd=1,nbdirs
    cos_soil_biotic_fw(nd) = temp*(pwr1*f_opt_fw(nd)+f_opt*pwr1_fw(nd)) &
&     + f_opt*pwr1*EXP(arg1)*arg1_fw(nd)
    cos_soil_fw(nd) = cos_soil_abiotic_fw(nd) + cos_soil_biotic_fw(nd)
  END DO
  cos_soil_biotic = f_opt*pwr1*temp
  cos_soil = cos_soil_abiotic + cos_soil_biotic
...
! arguments
  ...
  REAL(kind=8) :: x_fw(nbdirsmax, n), priordiff_fw(nbdirsmax, n)
! local
