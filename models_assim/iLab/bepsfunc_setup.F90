...
!         NOTE:
!         The implementation applies the
...
subroutine get_nmeteo(yr, mn, dy, tod, n_meteo)
  ...
  !
  !-- access reference time of meteorological forcing
  !
  call read_meteo_site_reftime(metyr, metmn, metdy)
  

  !-- compute difference in seconds with BEPS time manager
...
!     get_nlai
!
!> @brief procedure to determine for a given time point in
!         the BEPS time loop the corresponding temporal index in the
!         (daily) LAI forcing file for this day(if used)
!
!> \authors Michael Vossbeck, The Inversion Lab
!> \date    May 2024
!> \last    May 2024
subroutine get_nlai(yr, mn, dy, tod, n_lai)
  use shr_kind_mod, only: r8 =>shr_kind_r8
  use beps_time_manager, only:timemgr_datediff
  use controlInput_mod, only: read_lai_site_reftime
  use bepstype, only:bound
  implicit none
  !-- arguments
  integer, intent(in) :: yr, mn, dy, tod
  integer, intent(out) :: n_lai
  !-- local
  real(r8) :: days_lai !-- 
  integer  :: lai_yr, lai_mn, lai_dy
  integer :: iostat

  !-- access reference time of LAI ("days since")
  call read_lai_site_reftime(lai_yr, lai_mn, lai_dy)
  !-- compute difference in days with BEPS time manager
  !   (yr,mn,dy,tod) - (lai_yr,lai_mn,lai_dy,0)
  call timemgr_datediff(lai_yr*10000+lai_mn*100+lai_dy, 0, yr*10000+mn*100+dy, tod,&
       days_lai)

  !-- convert to integer
  !   iLab::looked-up and tested routine 'timemgr_datediff',
  !         the naming 'datediff' might be misleadding, as the
  !         difference returned ingesting the same day twice yields 1.
  !         Regarding the index of the time dimension in the NetCDF file,
  !         this is exactly what we need (no need to add 1 again as for the metforcing)
  n_lai = int(days_lai)

end subroutine get_nlai



!***********************************************************
!     disable_netcdf_output
!
!> @brief Disable NetCDF output when running BEPS model
!         (might help to speed up assimilation experiments,
!          where generation of NetCDF outputs is typically not required).
!
!> \authors Michael Vossbeck, The Inversion Lab
!> \date    September 2023
subroutine disable_netcdf_output()
  use mo_bepsfunc_ctl 
  implicit none
  enable_netcdf_out = .false.
end subroutine disable_netcdf_output


!***********************************************************
!     disable_write_restart
!
!> @brief Disables generation of restart files
!         (which is typically not needed when performing an assimilation experimen)
!
!> \authors Michael Vossbeck, The Inversion Lab
!> \date    September 2023
subroutine disable_write_restart()
  use mo_bepsfunc_ctl 
  implicit none
  write_restart = .false.
end subroutine disable_write_restart
