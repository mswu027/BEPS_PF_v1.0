...
subroutine evalf(n, x, m, y)
  ...
  integer :: jcnt !-- position counter in 1D target vector
  integer :: doys !-- number of days in (actual) year
  logical :: is_end_curr_year, is_end_curr_month, is_end_curr_day, is_first_step

  integer :: n_lai !-- index of time dimension in LAI NetCDF forcing

  !
  !-- consistency: multi-point serial setup 
  !
  if( nscale.ne.1 ) then
     write(*, '(a,(a,1x,i10))') ' FATAL::'//sub//':setup was *NOT* set for site-level/multi-point experiment!', &
          ' nscale=',nscale
     stop
  endif
  if( meteo_input.lt.0 ) then
     write(*, '(a)') ' FATAL::'//sub//':only hourly meteorological input'//&
          ' is supported.'
     stop
  endif
  if( lai_input.lt.0 ) then
     write(*, '(a)') ' FATAL::'//sub//':LAI is expected to be forcing input.'
     stop
  endif

  !-- size consistency of target vector
  if( m.ne.(nsimvar*npoints*ntp) ) then
     write(*, '(a,2(a,i6,1x))') ' FATAL::'//sub//':inconsistent sizes detected! ', &
          'expected m=',m,&
          'detected nsimvar*npoints*ntp=', nsimvar*npoints*ntp
     stop
  endif
  ...
  !-- map actual control vector to model parameters
  call x2beps(n,x)
     ...
     if (lai_input >=0) then
        ...
        if (is_first_step .or. is_end_curr_day) then
           if (nscale == 0) then
              ...
              !-- iLab::get index in time dimension of LAI in NetCDF matching the current day,
              !         (tod will be zero, except the simulation had been started subdaily)
              call get_nlai(yr, mn, dy, tod, n_lai)
...
!!! simulating Rh
              ...
              bound%Ccd(i,j) = Ccd(0)
              bound%Cssd(i,j) = Cssd(0)
              bound%Csmd(i,j) = Csmd(0)
              bound%Cfsd(i,j) = Cfsd(0)
              bound%Cfmd(i,j) = Cfmd(0)
              bound%Csm(i,j) = Csm(0)
              bound%Cm(i,j) = Cm(0)
              bound%Cm(i,j) = Cs(0)
              bound%Cp(i,j) = Cp(0) 
