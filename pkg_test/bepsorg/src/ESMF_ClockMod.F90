...
!EOP
      ...
      ALLOCATE(clockint%AlarmList(60))
...
! !ARGUMENTS:
      type(ESMF_Clock), intent(inout) :: clock
      type(ESMF_Alarm), dimension(60), intent(out), optional :: &
                                        RingingAlarmList
...
! !ARGUMENTS:
      ...
      type(ESMF_Alarm), dimension(60), intent(in) :: AlarmList
      integer, intent(out), optional :: rc
...
! !ARGUMENTS:
      ...
      type(ESMF_Time), intent(out) :: PrevTime
      integer(ESMF_KIND_I8), intent(out) :: AdvanceCount
      type(ESMF_Alarm), dimension(60), intent(out) :: AlarmList
      integer, intent(out), optional :: rc
...
!EOP
      ...
      print *, 'Start time: '
      call ESMF_TimePrint( start_time )
      print *, 'Stop time: '
      call ESMF_TimePrint( stop_time )
      print *, 'Reference time: '
      call ESMF_TimePrint( ref_time )
      print *, 'Current time: '
      call ESMF_TimePrint( curr_time )
      print *, 'Time step: '
