...
!      call flush(6)
      ...
      if (present(D)) then
         ...
         time%basetime%s = 86400_ESMF_KIND_I8 * INT(D-1,ESMF_KIND_I8)
         dset=.true.
      elseif (present(Dl)) then
         time%basetime%s = 86400_ESMF_KIND_I8 * Dl-1_ESMF_KIND_I8
         dset=.true.
      endif
...
!         write(6,*) 'tcx timeinc mon1 ',year,month,day,sec,nyr,nmon
         year  = year  + nyr
         month = month + nmon
         do while (month > 12_ESMF_KIND_I8)
            month = month - mpyi4
            year = year + 1
         enddo
         do while (month < 1)
...
SUBROUTINE timeaddmonths( time, MM, ierr )
  ...
  ierr = ESMF_SUCCESS
  IF ( ( MM < 1 ) .OR. ( MM > 12_ESMF_KIND_I8 ) ) THEN
