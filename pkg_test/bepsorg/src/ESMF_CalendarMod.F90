...
!------------------------------------------------------------------------------
      ...
      INTEGER, PARAMETER :: mday(12_ESMF_KIND_I8)   &
                          = (/31,28,31,30,31,30,31,31,30,31,30,31/)
      INTEGER, PARAMETER :: mdayleap(12_ESMF_KIND_I8) &
                          = (/31,29,31,30,31,30,31,31,30,31,30,31/)
      INTEGER, DIMENSION(365) :: daym
      INTEGER, DIMENSION(366) :: daymleap
      INTEGER :: mdaycum(0:12_ESMF_KIND_I8)
      INTEGER :: mdayleapcum(0:12_ESMF_KIND_I8)
      TYPE(ESMF_BaseTime), TARGET :: monthbdys(0:12_ESMF_KIND_I8)
      TYPE(ESMF_BaseTime), TARGET :: monthbdysleap(0:12_ESMF_KIND_I8)
      TYPE(ESMF_BaseTime), TARGET :: monthedys(0:12_ESMF_KIND_I8)
      TYPE(ESMF_BaseTime), TARGET :: monthedysleap(0:12_ESMF_KIND_I8)
...
!$$$ push this down into ESMF_BaseTime constructor
  ...
  DO i = 0,12_ESMF_KIND_I8
    j = i + 1
    if ( i == 12_ESMF_KIND_I8 ) j = 0
    monthedys(i)   = monthbdys(j)
    monthedys(i)%S = monthedys(i)%S - 86400_ESMF_KIND_I8
  ENDDO
...
!$$$ push this down into ESMF_BaseTime constructor
  ...
  DO i = 0,12_ESMF_KIND_I8
    ...
    monthedysleap(i)%S = monthedysleap(i)%S - 86400_ESMF_KIND_I8
  ENDDO
...
integer(esmf_kind_i8) FUNCTION nsecondsinyear ( year, calkindflag )
  ...
  nsecondsinyear = 86400_ESMF_KIND_I8 * INT( ndaysinyear(year, calkindflag) , ESMF_KIND_I8 )
...
integer function ndaysinmonth( year,month,calkindflag)
  ...
  IF ( ( MONTH < 1 ) .OR. ( MONTH > 12_ESMF_KIND_I8 ) ) THEN
...
integer(esmf_kind_i8) function nsecondsinmonth( year,month,calkindflag)
  ...
  nsecondsinmonth = ndaysinmonth(year,month,calkindflag)*86400_ESMF_KIND_I8
...
integer function ndayinyearsec(year, basetime, calkindflag)
  ...
  ndayinyearsec = ( tmpbasetime%S / 86400_ESMF_KIND_I8 ) + 1
